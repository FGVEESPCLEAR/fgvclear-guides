...
Dados de finanças públicas são fundamentais para a avaliação de políticas públicas por pelo menos quatro razões. Primeira, em várias áreas da política pública \textbf{o gasto é, em si, o instrumento}: transferências de renda, compras governamentais e investimentos em infraestrutura atuam sobre a realidade social via fluxo financeiro --- nesses casos, analisar dados orçamentários é analisar a política. Segunda, dados fiscais funcionam com frequência como \textbf{primeiro estágio} da análise causal: uma lei pode prever transferências ou obrigar determinado gasto, mas o que aparece na execução orçamentária pode diferir do previsto; antes de estimar efeitos de uma política sobre resultados sociais, é boa prática verificar que o gasto público associado efetivamente variou conforme a regra, sob pena de confundir ausência de efeito com ausência de implementação. Terceira, dados fiscais permitem calcular \textbf{custos de políticas}, insumo indispensável para análises de custo-efetividade e custo-benefício. Quarta, a \textbf{capacidade fiscal} dos entes federativos é um determinante importante da qualidade e da cobertura dos serviços públicos --- e, portanto, uma variável de contexto relevante em avaliações de políticas descentralizadas. Os dados de finanças públicas são ainda o principal insumo para o monitoramento do cumprimento da \textbf{Lei de Responsabilidade Fiscal} (Lei Complementar n.º 101/2000), permitindo verificar, por município ou estado, a observância dos limites de gasto com pessoal, de endividamento e dos mínimos constitucionais em saúde e educação --- e, assim, identificar entes sujeitos a sanções institucionais cujo comportamento fiscal pode interagir com as políticas avaliadas.
...
\small
\begin{tabularx}{\textwidth}{@{} >{\raggedright\arraybackslash}p{2.8cm} >{\raggedright\arraybackslash}p{3.2cm} >{\raggedright\arraybackslash}p{2.5cm} X @{}}
\toprule
...
\begin{itemize}
  ...
  \item \textbf{FINBRA histórico}: séries 1986--2012 disponíveis para download na STN. As séries mais antigas circulam em formatos legados (alguns exercícios em arquivos \texttt{.mdb} do Microsoft Access), o que pode exigir conversão para formatos abertos antes da análise; edições posteriores estão em Excel;
...
\small
\begin{tabularx}{\textwidth}{@{} >{\raggedright\arraybackslash}p{2.8cm} >{\raggedright\arraybackslash}p{3cm} >{\raggedright\arraybackslash}p{2.6cm} X @{}}
\toprule
\textbf{Fonte} & \textbf{O que registra} & \textbf{Cobertura} &
\textbf{Principal uso em avaliação} \\
\midrule
FINBRA/Siconfi   & Receitas e despesas anuais    & Municipal e estadual & Gasto por função, capacidade fiscal \\
RREO             & Execução bimestral            & Municipal e estadual & Acompanhamento infraanual \\
RGF              & Limites LRF                   & Municipal e estadual & Endividamento, risco fiscal \\
Portal Transparência & Execução orçamentária     & Federal            & Programas, transferências, beneficiários \\
SIOPE            & Gasto em educação             & Municipal e estadual & Custo-aluno, FUNDEB \\
SIOPS            & Gasto em saúde                & Municipal e estadual & Gasto per capita em saúde \\
TSE              & Financiamento eleitoral       & Municipal e estadual & Ciclo político, reforma eleitoral \\
