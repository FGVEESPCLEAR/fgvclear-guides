...
Censos são levantamentos que buscam cobrir \textit{toda} a população ou universo de interesse — não uma amostra, mas o conjunto completo de unidades. No Brasil, os principais censos relevantes para avaliação de políticas públicas são o Censo Demográfico (IBGE), o Censo Escolar (Inep) e o Censo Agropecuário (IBGE).
...
\textbf{Principais exemplos no Brasil.} Censo Demográfico (decenal), Censo Escolar (anual), Censo da Educação Superior (anual), Censo Agropecuário (periodicidade irregular).
...
\textbf{Limitações.} A limitação estrutural dos registros administrativos é que eles cobrem apenas o que passa pelo sistema formal. Trabalhadores informais não estão no CAGED. Partos domiciliares não registrados não estão no SINASC. Crianças fora da escola não constam no Censo Escolar. Essa é uma forma de viés de cobertura que pode ser especialmente problemática em avaliações de políticas voltadas a populações vulneráveis, que tendem a ter menor presença nos sistemas formais.
...
\textbf{Limitações.} A integração nem sempre é plena: sistemas diferentes dentro de um mesmo setor frequentemente não compartilham identificadores comuns, dificultando a linkagem entre bases. Além disso, a qualidade dos dados varia significativamente entre municípios e estados, refletindo diferenças na capacidade administrativa local.
...
Este capítulo organizou o ecossistema de fontes de dados em oito tipos, cada um com características, vantagens e limitações próprias. Os pontos centrais são:
...
Os próximos capítulos percorrem esse território fonte por fonte, começando pelas categorias mais consolidadas e avançando para as mais especializadas. A leitura pode ser feita em ordem ou por saltos temáticos --- a tipologia apresentada aqui serve de bússola em qualquer direção.

\medskip

O primeiro grande território a explorar é o das pesquisas amostrais e censos demográficos: as fontes que, desde o IBGE, nos dizem quem os brasileiros são, como vivem e em que condições --- o ponto de partida insubstituível de qualquer diagnóstico nacional.