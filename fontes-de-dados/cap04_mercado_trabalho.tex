...
\ficharapida
  ...
  {Brasil, Grandes Regiões, Unidades da Federação e municípios}
  {Anual (desde 1975)}
  {Vínculo empregatício (base de vínculos) e estabelecimento (base de estabelecimentos)}
...
As duas bases podem ser linkadas pelo CNPJ do estabelecimento, permitindo análises que combinam características dos trabalhadores e das firmas.
...
\begin{itemize}
  ...
  \item \textbf{Avaliação de políticas de qualificação profissional}: linkagem entre participantes de programas de treinamento (identificados pelo CPF) e seus vínculos na RAIS antes e depois do programa;
...
O NIS é o identificador único de cada pessoa no CadÚnico e pode ser usado para linkar o cadastro com bases de benefícios (Bolsa Família, BPC) e, mediante convênio, com outras bases administrativas.
...
\begin{itemize}
  ...
  \item \textbf{Linkagem com Bolsa Família}: o MDS disponibiliza dados de beneficiários do Bolsa Família (atual e histórico) no Portal da Transparência, com NIS como identificador.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Linkagem com registros de saúde e educação}: mediante convênio, o CadÚnico pode ser linkado com o SINASC, o SIM ou o Censo Escolar pelo CPF ou NIS, permitindo avaliar como condições socioeconômicas se associam a resultados de saúde e educação.
\end{itemize}

% CORRIGIDO: título encurtado para caber na caixa
\begin{boxexemplo}{Exemplo — Linkagem CadÚnico + Censo Escolar: Bolsa Família}
Uma das avaliações mais citadas do Programa Bolsa Família utilizou a linkagem entre o CadÚnico (que identifica as famílias beneficiárias pelo NIS) e o Censo Escolar (que registra a frequência de cada aluno pelo CPF). Ao combinar as duas bases, os pesquisadores puderam comparar a frequência escolar de crianças de famílias beneficiárias com a de crianças de famílias elegíveis mas não beneficiárias, controlando por características socioeconômicas observáveis do CadÚnico. Essa linkagem tornou possível uma avaliação robusta sem necessidade de coleta primária de dados.
...
\addlinespace
A política aumentou a taxa de formalização entre trabalhadores de baixa renda? & CadÚnico + CAGED/RAIS (linkagem por CPF) \\
\addlinespace
...
\begin{itemize}
  ...
  \item A \textbf{RAIS} registra o estoque anual de vínculos formais, incluindo estatutários, e é a principal fonte para análises de diferencial salarial, mobilidade ocupacional e características de firmas; o acesso com CPF, via convênio, abre possibilidades de linkagem com outras bases;
  ...
  \item A \textbf{combinação estratégica} dessas fontes — especialmente via linkagem por CPF ou NIS — permite análises muito mais ricas do que qualquer fonte isolada.
\end{itemize}

O mercado de trabalho registra quem tem emprego formal — mas não registra o que acontece quando esse trabalhador adoece, se acidenta ou deixa de ter acesso ao sistema de saúde. A dimensão sanitária da vida das pessoas é ao mesmo tempo causa e consequência das condições de trabalho e renda. O próximo capítulo abre esse universo, apresentando os sistemas do DATASUS e a Pesquisa Nacional de Saúde — fontes que cobrem nascimentos, mortes, internações e a experiência de saúde de toda a população brasileira, formal ou informal.