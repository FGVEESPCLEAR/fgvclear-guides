...
\begin{itemize}
  ...
  \item \textbf{R}: o pacote \texttt{rvest} oferece funcionalidades de \textit{scraping} integradas ao ecossistema do \texttt{tidyverse}; para requisições HTTP mais estruturadas (APIs REST, autenticação, \textit{headers} customizados, controle de \textit{cookies}), o pacote \texttt{httr} (ou \texttt{httr2}) complementa o \texttt{rvest}, cumprindo papel análogo ao da biblioteca \texttt{requests} em Python;
...
\begin{boxdica}{Nota --- Revisão de literatura e síntese de evidências}
O uso de IA e de ferramentas digitais para revisão de literatura e síntese de evidências --- incluindo plataformas como \textit{Elicit}, \textit{Consensus} e \textit{Semantic Scholar} --- é tratado em detalhe no guia \textit{Uso de Evidências Acadêmicas em Avaliação de Políticas Públicas}, também integrante da série \textit{Avaliação na Prática}. Este capítulo concentra-se nas tecnologias de \textbf{acesso e coleta de dados} (APIs, \textit{web scraping}, IA aplicada à estruturação de microdados).
\end{boxdica}
...
Todas essas ferramentas e fontes --- tradicionais e emergentes, nacionais e internacionais --- convergem para um momento concreto: o de escolher um dado específico, obtê-lo, combiná-lo com outras bases e usá-lo para responder uma pergunta avaliativa real. O capítulo final reúne os fios condutores do guia em um roteiro prático para esse processo --- da pergunta à fonte, do acesso à documentação, da integração à conformidade com a LGPD.