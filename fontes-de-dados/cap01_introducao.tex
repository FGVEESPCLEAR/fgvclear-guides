...
A escolha da fonte de dados não é uma decisão técnica menor --- é um passo metodológico com consequências diretas sobre a validade das conclusões de qualquer avaliação. Este capítulo apresenta os princípios que orientam essa escolha: o que distingue dados bons de dados ruins para uma determinada pergunta avaliativa, como pensar a complementaridade entre diferentes tipos de fontes, e o que o avaliador precisa verificar antes de depositar confiança em qualquer base. O capítulo também oferece uma visão panorâmica do ecossistema de dados disponíveis para avaliação de políticas públicas no Brasil.
\end{boxdestaque}
...
Registros administrativos são dados coletados como subproduto da operação de sistemas governamentais ou privados: cada empregado registrado na RAIS, cada internação no SIH, cada aluno matriculado no Censo Escolar, cada beneficiário do Bolsa Família no CadÚnico. Esses dados não foram coletados para fins de pesquisa, mas para gerir programas e sistemas.
...
Identificada a categoria de fonte potencialmente adequada para uma pergunta avaliativa, o passo seguinte é verificar se uma fonte específica é realmente adequada. Oito critérios organizam essa verificação.
...
A unidade de análise da fonte é compatível com a da pergunta? Se a pergunta é sobre alunos individuais, é preciso microdados de alunos (como o Censo Escolar), não dados agregados por escola ou município. Se a pergunta é sobre estabelecimentos de saúde, o CNES é mais adequado do que o SIM. Trabalhar com unidades de análise incorretas leva a falácias ecológicas e erros de inferência.
...
\subsection{Identificadores para linkagem}

A fonte tem identificadores que permitem linkar com outras bases de que o analista precisa? CPF, NIS, CNPJ, código de escola do Inep e código de estabelecimento de saúde (CNES) são os principais identificadores que viabilizam linkagens no ecossistema de dados brasileiro. A disponibilidade desses identificadores --- e as restrições de acesso que os acompanham --- é uma dimensão crítica na fase de planejamento.
...
Outros produtores & TSE, INPE, SNIS, Ipea & Eleitoral, ambiental, saneamento, indicadores derivados \\
\bottomrule
\end{tabularx}
...
% ────────────────────────────────────────────────────────────
\section{Complementaridade e linkagem entre fontes}
% ────────────────────────────────────────────────────────────

Uma das características mais importantes --- e mais subutilizadas --- do ecossistema de dados brasileiro é a possibilidade de \textbf{linkar bases de dados} por meio de identificadores individuais. O CPF vincula pessoas físicas em qualquer base que o contenha. O NIS conecta beneficiários de programas sociais ao CadÚnico. O código de escola do Inep está presente no Censo Escolar e no SAEB. O CNES identifica estabelecimentos de saúde no SIH, no SIA e no SINAN.

Essas linkagens permitem construir perfis multidimensionais que nenhuma fonte isolada poderia fornecer. Um exemplo clássico: linkar o CadÚnico com o Censo Escolar pelo número de identificação do aluno permite analisar simultaneamente as condições de vida da família (renda, composição familiar, acesso a serviços) e o desempenho e a trajetória escolar das crianças. Linkar a RAIS com dados de mortalidade do SIM pelo CPF permite estimar o impacto da formalização do emprego sobre expectativa de vida.

A capacidade de fazer essas linkagens é, em muitos casos, o que torna avaliações causais rigorosas possíveis no Brasil. O Capítulo~13 discute as boas práticas e as obrigações legais que acompanham o uso de linkagens de bases.
...
\begin{itemize}
  ...
  \item Oito critérios estruturam a avaliação de adequação de uma fonte: cobertura geográfica, periodicidade, unidade de análise, representatividade, granularidade temática, identificadores para linkagem, qualidade do preenchimento e documentação metodológica;

  \item O ecossistema brasileiro é organizado em torno do \textbf{IBGE}, dos \textbf{sistemas setoriais} dos ministérios e das \textbf{agências reguladoras e outros produtores especializados} --- cada pilar com fontes distintas e complementares;

  \item A \textbf{linkagem entre bases} por identificadores individuais é um dos ativos mais poderosos do ecossistema brasileiro, viabilizando análises multidimensionais que nenhuma fonte isolada permitiria.
\end{itemize}

\medskip

Para navegar esse ecossistema com profundidade, é preciso primeiro entender as diferentes categorias de fontes e as lógicas que as distinguem. O capítulo seguinte oferece exatamente isso: uma tipologia sistemática que organiza o território antes que percorramos cada região temática.