...
\small
\begin{tabularx}{\textwidth}{@{} p{3cm} p{3.5cm} >{\raggedright\arraybackslash}p{2.5cm} X @{}}
\toprule
...
Esses dados permitem construir séries históricas municipais de cobertura (número de famílias beneficiárias) e valor médio dos benefícios, além de análises individuais quando cruzados com o CadÚnico pelo NIS.
...
\begin{itemize}
  ...
  \item \textbf{Impacto sobre saúde dos beneficiários}: integração com dados do DATASUS (SIH, SIM) para analisar efeitos sobre hospitalização e mortalidade de idosos e PcD;
...
A Tarifa Social de Energia Elétrica concede descontos na conta de luz para famílias de baixa renda cadastradas no CadÚnico. Os dados de beneficiários estão disponíveis na ANEEL e podem ser cruzados com o CadÚnico pelo NIS para análises de cobertura e focalização.
...
A avaliação de políticas de assistência social e transferência de renda se beneficia enormemente da \textbf{integração entre bases}. O CadÚnico, por usar o NIS como identificador único de cada pessoa, serve como elo central que conecta:
...
\begin{itemize}
  ...
  \item \textbf{CadÚnico $\leftrightarrow$ DATASUS}: integração por CPF ou NIS permite analisar desfechos de saúde (mortalidade, internações, vacinação) em função do recebimento de benefícios;
...
\begin{itemize}
  \item O \textbf{CadÚnico} é a espinha dorsal do sistema de dados de assistência social: serve como porta de entrada para os principais programas e como base para integração com outras fontes;
  ...
  \item A \textbf{integração entre bases} — especialmente CadÚnico com Censo Escolar, DATASUS e RAIS — é o que permite as avaliações mais robustas de políticas sociais.
\end{itemize}

O próximo capítulo aborda as fontes de dados sobre segurança pública: o Atlas da Violência, o Anuário FBSP, os registros estaduais (especialmente SP, RJ e MG) e as pesquisas de vitimização --- conjunto de fontes heterogêneo, com problemas estruturais de comparabilidade entre estados que exigem cautela metodológica especial.