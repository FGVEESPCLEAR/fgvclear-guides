...
\usepackage{amsmath,amsthm,amssymb, dsfont, mathtools, amsfonts}
\usepackage{graphicx, xcolor}
\usepackage[portuguese]{babel} % Define o idioma como português
\usepackage{enumitem}
\usepackage{comment}
\usepackage{float}
\usepackage{hyperref}
...
\usepackage{tcolorbox}
 % Codificação do documento em UTF-8
\usepackage{listingsutf8}      % Suporte para UTF-8 no listings

% Configuração do ambiente de listagem com caixa e UTF-8
...
\lstdefinestyle{RStyle}{
    ...
    numbers=none,                       % Sem numeração de linha
    xleftmargin=0pt,                    % Margem esquerda mínima
    frame=none,                         % Sem borda do listings
    inputencoding=utf8,                 % Define a codificação UTF-8
    literate={á}{{\'a}}1 {â}{{\^a}}1 {ã}{{\~a}}1 % Mapeamento manual para caracteres acentuados
             {é}{{\'e}}1 {ê}{{\^e}}1 {í}{{\'i}}1
             {ó}{{\'o}}1 {õ}{{\~o}}1 {ô}{{\^o}}1
             {ú}{{\'u}}1 {ç}{{\c{c}}}1 {Á}{{\'A}}1
             {É}{{\'E}}1 {Í}{{\'I}}1 {Ó}{{\'O}}1
             {Ú}{{\'U}}1 {Ç}{{\c{C}}}1, % Mapeamento para outros caracteres
...
% Definindo a caixa para o código R
\newtcblisting{RCodeBox}[1][]{
    listing only,
    listing options={style=RStyle, language=R},
    colframe=black,                     % Cor da borda da caixa
    colback=gray!10,                    % Cor de fundo cinza claro
    sharp corners,                      % Cantos retos
    boxrule=0.5pt,                      % Espessura da borda
    fonttitle=\bfseries,
    title=#1,
    arc=0mm,                            % Remove arredondamento
    width=\linewidth                    % Preenche toda a largura da linha
}
...
\addbibresource{bibliography.bib}
    ...
    %to remove unnecessary commas
    \DefineBibliographyExtras{english}{\let\finalandcomma=\empty}

%Theorems, assumptions and propositions
...
% Configuração do ambiente de definição
\newtheorem{definition}{Definição}[section]
% Configuração do ambiente de teorema
...
%Short-cut for recurrent math commands
...
Suponha que o governo do seu país decida aplicar uma taxa de $20\%$  sobre compras internacionais de até $50$ dólares. Você deseja estimar o impacto dessa medida sobre os consumidores brasileiros, especialmente quanto essa taxa representará em relação à renda da população. A primeira (e mais simples) abordagem para estimar o custo ao consumidor é calcular de que forma a tributação afetaria o valor médio das remessas internacionais abaixo de $50$ dólares. 

Após uma rápida busca na internet, você descobre que o valor médio das remessas internacionais entre setembro de 2023 e janeiro de 2024 é de, aproximadamente, R\$ $78$.\footnote{\textcite{folha_remessas}.} Com esse dado em mãos, você calcula que o consumidor brasileiro pagaria R\$ $15,.6 \ (20\% \timescdot 78)$ por remessa. Você também calcula que, por mês, o brasileiro médio faz, aproximadamente, $0,77$ compras internacionais de menos de $50$ dólares. Consequentemente, o brasileiro médio gastaria pouco mais de R\$ $12$ por mês, com a imposição da nova taxa. Em princípio, esse número parece pequeno em relação a, por exemplo, o rendimento domiciliar per capita brasileiro em 2023, que foi de R\$ $1.893$.\footnote{\textcite{poder360rendadomiciliar}.}

Porém, ao refletir sobre essa estimativa, você questiona tanto a abordagem por meio da qual ela foi obtida, como também o quanto ela de fato reflete a realidade. Com uma pesquisa mais aprofundada, você descobre que o Imposto sobre Circulação de Mercadorias e Prestação de Serviços (ICMS) incidirá sobre o preço da compra após a taxa de $20\%$. Ou seja, a tributação teria um custo adicional indireto em virtude o valor de ICMS cobrado. 

Além disso, você suspeita que compras internacionais de até $50$ dólares são feitas por apenas uma parcela da população brasileira, de modo que o seu exercício original ignora completamente a heterogeneidade entre os consumidores. Se esse tipo de compra estiver concentrado em um grupo específico, então parte da população fará (bem) mais do que 0,77 compras por mês. Além disso, se a maioria desses consumidores for de menor renda, o impacto da tributação representará uma fração ainda maior de sua renda. Diante dessas considerações, você começa a refletir sobre como utilizar dados que relacionem a distribuição de renda da população com o valor e o número de remessas realizadas mensalmente. 

Dando continuidade à sua linha de raciocínio, você percebe que, mesmo com dados ideias, c— contendo renda mensal, número e valor de remessas para cada brasileiro —, a introdução do tributo provavelmente alteraria o comportamento dos consumidores. Isso porque o aumento do preço provocado pela taxação tenderia a reduzir a demanda por compras internacionais. DPara quantificar esse efeito, seria necessário estimar a função de demanda por esse tipo de consumo. 


\hfill\break

\noindent O problema acima indica algumas direções e, sobretudo, dificuldades ao prever o efeito causal de políticas públicas. Por exemplo, ao estimar que a taxa vai gerar um custo adicional de R\$ $12$ por mês, ignorando que o ICMS incide sobre esse valor, estamos ignorando como a nova política interage com as políticas existentes. Além disso, ao considerar apenas o efeito sobre a \textit{remessa média} e sobre o \textit{brasileiro médio}, estamos desconsiderando que a política afeta pessoas diferentes de formas diferentes, e que o seu efeito pode ser bastante heterogêneo. Adiciona-se a isso o fato de que pessoas mudam seu comportamento após a implementação de uma política, reagindo assim ao novo conjunto de incentivos. 

Ao mesmo tempo, há limitações impostas pela disponibilidade de dados. Por exemplo, se quisermos estender o exercício simples aos diferentes quartis da distribuição de renda no Brasil, precisaríamos de informações mais detalhadas — como o número e o valor das compras internacionais para cada faixa de renda. No entanto, esses dados nem sempre estão disponíveis. De forma geral, quanto mais complexo o exercício, maior a necessidade de dados granulares. 

Neste guia, descrevemos uma metodologia conhecida como \textbf{microssimulação}, que pode ser utilizada para prever os efeitos de políticas públicas. Em linhas gerais, essa abordagem consiste em construir um modelo que represente os aspectos relevantes da política — como a interação com outras políticas existentes e os efeitos sobre o comportamento dos agentes — e, a partir disso, simular o que ocorreria com as variáveis de interesse caso a política fosse implementada.  Apresentamos uma definição mais precisa e uma descrição mais detalhada na Subseção \ref{sec:microssimulacao_intro}. Antes disso, na Subseção \ref{sec:contrafactual_geral}, consideramos uma estrutura formal para o problema de previsão do efeito de uma política pública, que será a base para toda a exposição subsequente.

Em seguida, abordamos duas temáticas importantes: Escolha Discreta e Modelos Dinâmicos, nas seções \ref{sec:discrete_choice} e \ref{sec:model_din}, respectivamente. Esses dois arcabouços teóricos constituem a base da maioria dos exercícios de microssimulação. Por fim, concluímos o guia com três exemplos de aplicação da metodologia a políticas públicas, incluindo a programação utilizada para estimar os resultados. 


\subsection{Uma formulação geral para o problema}
\label{sec:contrafactual_geral}

O modelo descrito abaixo é baseado em \textcite{figari2015}.

Considere que estamos interessados em comparar duas políticas distintas e mutuamente excludentes, uma política \textit{Base} ($B$) e uma política \textit{Contrafactual} ($C$). Seja $p$ o indexador para política, de forma que $p\in\{B, C\}$, e seja $Y_p$ a variável resultado de interesse no cenário em que a política $p$ é implementada. Nós gostaríamos de comparar $Y_B$ com $Y_C$. O problema fundamental é que é impossível observar $Y_B$ e $Y_C$ ao mesmo tempo.\footnote{Para os familiarizados com métodos quantitativos de avaliação de impacto, $Y_B$ e $Y_C$ são o que chamamos de \textit{resultados potenciais}.} Daqui em diante, para facilitar a exposição, vamos considerar que a política em vigor no momento da coleta dos dados é a política $B$, e que estamos interessados nos efeitos da política $C$ \textbf{antes} de sua implementação. Nesse caso, somos capazes de observar $Y_B$, mas não $Y_C$. 

Note aqui uma diferença fundamental entre as análises \textit{ex-ante} e as análises \textit{ex-post}. Nas análises \textit{ex-post}, em geral exploramos que a política é aplicada a um subgrupo da população e que, dessa forma, observamos $Y_B$ para as pessoas que estão sob efeito da política $B$ e observamos $Y_C$ para as pessoas que não estão sob efeito da política $C$. Sob hipóteses apropriadas, por exemplo supondo que esses dois tipos de pessoas são comparáveis, conseguimos obter comparações válidas entre $Y_B$ e $Y_C$. Porém, isso não ocorre na análise \textit{ex-ante}, em que justamente queremos comparar $Y_B$ e $Y_C$ antes da implementação da política $C$,  no contexto estudado. Portanto, em essência, o que queremos é prever $Y_C$ ou, pelo menos, características de $Y_C$ que consideramos relevantes (sua média por exemplo). 

Uma forma para avançar na previsão de $Y_C$ é considerar um \textit{modelo} para a variável de interesse, e usar que observamos $Y_B$ para aprender sobre esse modelo. Formalmente, um modelo para $Y_p$ é um trio $(g_p, X_p, \theta_p)$ em que $g_p$ é uma função, $X_p$ são variáveis, observáveis ou não, que determinam $Y_p$ e/ou pelas quais a política afeta $Y_p$, e $\theta_p$ são os parâmetros da política $p$, de forma que
\begin{equation}
\label{eq:model}
Y_p=g_p(X_p, \theta_p).
\end{equation}

Devemos entender $\theta_p$ como as características que descrevem a política de forma que, em geral, $\theta_p$ é conhecido tanto para a política $B$ como para a política $C$. Por outro lado, embora a parte observável de $X_B$ esteja presente nos dados, não conseguimos observar $X_C$ pelo mesmo motivo que não observamos $Y_C$. Por fim, $g_p$ deve ser entendida como a estrutura que combina as variáveis $X_p$ com os parâmetros da política $\theta_p$ para gerar o resultado $Y_p$. Uma política pode afetar a estrutura $g_p$ de formas conhecidas antes de sua implementação, mas também de formas não conhecidas. 
...
Primeiro, vamos voltar ao caso da tributação de compras internacionais descrito na introdução. Estávamos interessados em entender como a taxa afetaria a renda da população. Isto é, queríamos saber a diferença entre a renda nos cenários sem taxa e com taxa de $20\%$. Nesse caso, $B$ indexa o cenário sem a taxa e $C$ indexa o cenário com a taxa. A nossa variável de interesse é a renda da população \textbf{após} a taxa, de forma que $Y_B$ e $Y_C$ são as rendas após a incidência nos cenários $B$ e $C$ respectivamente. 

Para computar a renda da população após a tributação, o caminho natural é descontar o valor gasto com a taxa da renda pré-taxação. Para isso, precisaríamos da renda pré-taxação, da quantidade de compras internacionais, do valor de cada compra internacional e do valor da taxa. Também, porque o ICMS incide sobre a taxa, o valor do ICMS é um fator de primeira ordem na hora de computar os custos gerados pela taxa. 

Note que tanto o valor da taxa como o valor do ICMS são parâmetros da política, de forma que é natural considerar $\theta_p=(\tau^{CI}_p, \tau^{ICMS}_p)$, em que $\tau^{CI}_p$ é a taxa, em $\%$, incidente sobra as compras internacionais no cenário $p$ e $\tau^{ICMS}_p$ é o ICMS cobrado sobre compras internacionais no cenário $p$, também em $\%$. Dessa forma, temos que $\theta_B=(0, 17\%)$ e $\theta_C=(20\%, 17\%)$ neste exemplo.\footnote{\textcite{folha_remessas}.} Note aqui uma hipótese de que o ICMS não mudaria após a introdução da taxa. 

Já a renda pré-taxação, a quantidade de compras internacionais e o valor das compras internacionais são variáveis que explicam $Y_p$ e pelas quais a política afeta $Y_p$, de forma que essas seriam as variáveis incluídas em $X_p$. Note que, a princípio, seria possível observar $X_B$ (em um mundo de dados ideais pelo menos), mas não é possível observar $X_C$ diretamente. 
...
\end{equation}
Note, porém, que isso depende crucialmente da hipótese de que a política não afeta nem a renda pré-taxação, nem o valor das compras internacionais. 

Em relação a $V_C$, tal hipótese será tão mais forte quanto mais  os preços e a quantidade demandada forem afetados pela taxação. Em geral, como esperamos que os consumidores reajam demandando menos de um produto quando este é taxado, a hipótese que $V_C=V_B$ parece particularmente forte. O custo de flexibilizá-la é que precisaríamos adicionar um novo modelo, agora para explicar $V_p$. Isso deixa claro um trade-off entre complexidade da análise e quão bem ela se aproxima da realidade. 

Por outro lado, a hipótese que $I_C=I_B$ parece mais razoável. É verdade que, a princípio, a taxação de compras internacionais pode afetar a renda pré-taxação das pessoas. Por exemplo, os consumidores podem começar a comprar mais produtos nacionais. Também é possível que haja uma expansão na demanda por trabalho, elevando os salários da população. Ou ainda, por alterar o fluxo de comércio do país com o restante do mundo, a taxa pode ter efeito no câmbio, que por sua vez pode afetar a renda. Entretanto, esses efeitos parecem ser de segunda ordem quando comparados com a incidência da taxação e o ajuste na demanda. Por isso, pode ser que $I_C=I_B$ seja de fato uma hipótese que estamos dispostos a considerar. Caso quiséssemos, por exemplo, permitir o efeito em salários devido à expansão da produção nacional, precisaríamos adicionar um modelo para o mercado de trabalho desse país. Novamente aqui aparece o trade-off entre complexidade e realismo do modelo.
...
O governo do seu país está considerando incluir um novo remédio, o WZ, no sistema público de saúde,  através do qual o medicamento seria distribuído gratuitamente. O objetivo do WZ é combater uma doença específica. Essa doença tem dois estágios, um menos grave, o outro mais grave. Pessoas em estágio mais grave são internadas em centros intensivos de tratamento por bastante tempo, e podem eventualmente falecer. 

Estudos sobre o medicamento WZ mostram que ele é bastante efetivo em prevenir que  pessoas em estado menos crítico evoluam para um quadro grave. Porém, ele não é muito efetivo em tratar pacientes que já se encontram em estágio grave da doença.

Um dos fatores que o governo está levando em consideração na decisão de  prover o medicamento via o sistema público é o quanto seria economizado com a redução de internações, em virtude de um menor número de doentes em estado grave. Ou seja, o governo está interessado em uma previsão do efeito da adoção do remédio no custo de internação nos próximos anos. 

Nesse contexto, a variável de interesse $Y_p$ é o custo de internação, que crucialmente vai depender do custo diário de internação e do número de dias que a pessoa fica internada. O número de dias que a pessoa fica internada irá variar de pessoa a pessoa, por isso essa é uma variável que entra em $X_p$. Por outro lado, o custo diário de internação é, basicamente, o custo para o governo em manter um leito em um centro de tratamento intensivo. Isso pode ser considerado um parâmetro da política. Porém, como o efeito do medicamento em estados avançados da doença é irrelevante, faria sentido supor que tanto o tempo de internação como o custo diário não mudam após a adoção do remédio. Mas então, como o remédio afeta $Y_p$?

Embora possa fazer sentido supor que o remédio não afeta o custo por doente grave, note que o custo total para o governo depende do número de doentes graves, e nesse ponto o remédio causa um impacto. Na verdade, o medicamento afeta a probabilidade de transição do estado menos grave para o estado mais grave da doença. Por isso, $g_p$ deve incluir um modelo para a transição entre estados, levando em conta que a probabilidade de transição do estado menos grave para o estado mais grave será alterada pela utilização do remédio por usuários do sistema público de saúde. 

Como parâmetros da política, seria interessante incluir, além do custo diário do leito, o efeito do remédio na probabilidade de transição. Note que, diferente dos Exemplos \ref{ex:ci_taxation} e \ref{ex:cct_school}, esse valor não é definido pela política, mas nós temos acesso a uma estimativa produzida pela literatura especializada.

Com esse modelo de transição, conseguimos simular o número de doentes graves, e com o número de doentes graves e o custo médio de internação por doente grave, conseguiríamos uma estimativa da redução dos custos de internação em razão da política. 

Porém, seria possível estimar esse efeito de forma mais precisa, tornando o modelo mais complexo para capturar aspectos importantes da realidade. Por exemplo, o fato de o remédio ser ofertado gratuitamente não significa que todas as pessoas em estado menos avançado tomam o remédio. Pode ser interessante modelar essa heterogeneidade na adoção do WB. Uma forma de fazer isso é utilizar modelos para variáveis binárias (como descrito na Seção \ref{sec:discrete_choice_bv}), considerando quais variáveis observáveis explicam a adoção heterogênea do remédio, como por exemplo gênero, idade, escolaridade e renda. Variáveis que, então, entrariam em $X_p$. Inclusive, o número de dias que a pessoa fica internada também podem depender dessas variáveis. Pense, por exemplo, que pessoas com idade mais avançada ficam mais tempo internadas, mas são justamente essas pessoas que mais adotam o remédio. Levar essa dimensão em conta pode ser importante para se ter uma noção mais precisa dos efeitos da política. 
\end{exemplo}
...
\subsection{Modelo de variável aleatória aditiva}
...
\subsection{Modelos Multinomiais}
...
\section{Modelos Dinâmicos}\label{sec:model_din}
...
\]
para todos \( x_0, \dots, x_{n-1}, i, j \in S \), e \( n \geq 0 \). O conjunto \( S \) é o espaço de estados da cadeia de Markov. Uma cadeia de Markov é chamada finita se o conjunto $S$ tem um número finito de estados.
\end{definition}
...
Além da distribuição marginal, com uma distribuição inicial podemos chegar à distribuição conjunta:
...
\subsection{Taxação de importados}
\label{sec: exemplo_1}
...
\subsection{Inclusão de Remédio no Sistema Público de Saúde\footnote{Esta seção é baseada no artigo "Microsimulation Modeling for Health Decision Sciences Using R: A Tutorial" de Krijkamp et al. (2018).}}



Retomando o nosso exemplo sobre saúde pública, vamos agora usar os conhecimentos sobre cadeias de Markov para construir um modelo dinâmico que avalie o impacto ex-ante da introdução de um medicamento no sistema público de saúde. A análise considera tanto os benefícios financeiros como a redução nos custos de internação, além dos ganhos em qualidade de vida para os pacientes, medidos por métricas como QALYs (Quality-Adjusted Life Years).
...
Para modelar a trajetória de saúde de cada indivíduo ao longo do tempo, usamos cadeias de Markov não homogêneas com probabilidades de transição ajustadas para refletir os efeitos do remédio. Essas probabilidades são dinâmicas, incorporando fatores como a duração dos estados $S1$ e $S2$, que aumenta o risco de mortalidade com o tempo. Adicionalmente, o modelo captura o impacto econômico e os benefícios de saúde pública através da integração de custos descontados e ganhos de utilidade ajustados pela qualidade de vida.

Por meio dessa abordagem, será possível estimar os custos totais e os QALYs médios por indivíduo, de modo a avaliar a viabilidade econômica e os benefícios sociais da implementação dessa política de saúde.

Esse exemplo será construído em cinco partes. A primeira irá expor a matriz de transição de probabilidades para os diferentes ciclos temporais. A segunda parte irá descrever os custos do modelo. A terceira irá expor  como os agentes julgam os benefícios da nova política. A quarta irá expor o algoritmo do modelo, e a quinta irá realizar um exercício quantitativo.

\subsubsection{Matriz de transição}
...
Inicialmente, todos os indivíduos são considerados saudáveis (H). Ou seja a $P(M_{i0} = H) = 1 ;  \forall i $. Em outras palavras, a distribuição inicial da cadeia é $\alpha^{(0)} = (p_H,p_{S1},p_{S2},p_{D}) = (1,0,0,0) $.
...
Seguindo o modelo contido em  Krijkamp et al(2018), os valores dos parâmetros serão os seguintes:
\begin{itemize}
    \item Probabilidade de morte quando saudável: \(p_{HD} = 0.005\)
    \item Probabilidade de permanecer doente quando saudável: \(p_{HS1} = 0.15\)
...
Se um gestor estiver interessado em saber qual foi o custo médio de cada paciente, levando em conta todos os ciclos temporais,, isso pode ser feito pelo custo médio na coorte:
...
\subsubsection{Aplicação Numérica}
...
Os custos médios por paciente foram R\$62.667 na ausência de tratamento e R\$117.455 quando há tratamento, resultando em um custo incremental médio de tratamento de R\$54.787. Por outro lado, os QALYs médios foram 15,279 na ausência de tratamento e 15,786 quando há tratamento, indicando um ganho incremental médio de 0,507 QALYs por paciente ao se introduzir o tratamento.

Com esses valores, o ICER (Incremental Cost-Effectiveness Ratio), calculado como a razão entre os custos e os benefícios incrementais, foi estimado em 108.047 reais por QALY. Ou seja, o custo médio de ganho de um ano adicional ajustado pela qualidade de vida ao implementar o tratamento é de R\$108.047.

Contudo, é possível que haja uma incerteza sobre os custos do tratamento. É interessante, portanto, considerar a possibilidade deste exceder os R\$12.000 originalmente considerados. A Tabela \ref{tab:icer_results_2} revela que, caso o custo do tratamento seja de R\$15.000 ou de R\$20.000, o custo incremental médio de oferecer tratamento por pessoa é de R\$64.484 e R\$91.312, respectivamente. Isso representa 112\% e 166\% do custo incremental originalmente calculado. Evidentemente, o ICER também aumenta, passando para R\$134.983 e R\$179.977 o custo médio de ganho de um ano adicional, ajustado pela qualidade de vida ao implementar-se o tratamento.
...
\subsection{Estimação modelos variáveis binárias}
