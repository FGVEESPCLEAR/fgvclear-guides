...
x  <- cutoff1 - renda_raw   # running variable centralizada em R$218 (elegíveis: x > 0)
x2 <- cutoff2 - renda_raw   # running variable centralizada em R$109 (elegíveis: x > 0)

# Covariáveis (para testes de balanceamento)
# Distribuição independente do lado do cutoff — garante continuidade das covariáveis
num_filhos        <- rpois(N, lambda = 2.3)
escolaridade_resp <- pmin(pmax(round(rnorm(N, mean = 6.5, sd = 2.5), 1), 0), 16)
area_urbana       <- rbinom(N, 1, prob = 0.70)
...
fig1 <- ggplot() +
  ...
  scale_color_manual(
    values = c("Elegível (direita)" = clear_teal,
               "Não elegível (esquerda)" = clear_navy), name = NULL) +
  scale_shape_manual(
    values = c("Elegível (direita)" = 16,
               "Não elegível (esquerda)" = 17), name = NULL) +
  annotate("text",
  ...
  labs(
    ...
    x        = "Renda per capita centralizada (R$ 218 − R$)",  # positivo = elegível
...
dens_e <- density(df$x[df$x >= 0   & df$x < 150], bw = 12)
dens_d <- density(df$x[df$x > -150 & df$x < 0],  bw = 12)

df_dens <- bind_rows(
  data.frame(x = dens_e$x, y = dens_e$y, lado = "Elegível (direita)"),
  data.frame(x = dens_d$x, y = dens_d$y, lado = "Não elegível (esquerda)")
) |> filter(x >= -150, x <= 150)
...
pA <- ggplot(db3, aes(x = xm, y = tm, color = lado, shape = lado)) +
  ...
  scale_color_manual(
    values = c("Elegível (direita)" = clear_teal,
               "Não elegível (esquerda)" = clear_navy), name = NULL) +
  scale_shape_manual(
    values = c("Elegível (direita)" = 16, "Não elegível (esquerda)" = 17),
    name = NULL) +
