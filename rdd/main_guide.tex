...
%%  Guia de Regressão Descontínua (RDD)
%% ============================================================
...
\hypersetup{
  ...
  pdfauthor={FGV CLEAR},
  pdftitle={Guia de Regressão Descontínua (RDD)}
}
\usepackage{natbib}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[R]{\small\color{clearblue}\textit{Guia CLEAR $|$ Regress\~ao Descont\'inua (RDD)}}
\fancyfoot[C]{\thepage}
...
% ==========================================
\thispagestyle{empty}
\chapter*{Ficha T\'ecnica}

\textbf{Equipe t\'ecnica:} Andr\'e Portela Souza, Caio de Souza Castro, Fabr\'icia Volot\~ao Peixoto, Luigi Garzon, Lycia Lima.
...
O Bolsa Família tem uma linha de renda. Famílias abaixo dela entram no
programa, famílias acima ficam de fora. Suponha que você queira saber se
receber o benefício afeta a frequência escolar dos filhos. A comparação
direta entre o grupo que recebe e o grupo que não recebe vai ser enviesada
por construção: as famílias beneficiárias são, em média, mais pobres, têm
menos escolaridade, vivem em municípios diferentes, têm trajetórias
laborais diferentes. Tudo isso afeta a frequência escolar mesmo sem o
programa. A diferença observada mistura efeito da pobreza com efeito do
benefício, e nenhuma análise séria pode ignorar isso.

A regressão descontínua se aproveita de uma característica do desenho do
programa para contornar o problema. A regra de elegibilidade é abrupta:
R\$ 5 abaixo da linha entra, R\$ 5 acima fica de fora. Mas duas famílias
separadas por R\$ 10 na renda mensal não são, salvo pelo benefício,
populações distintas. Idade dos filhos, escolaridade dos pais, região onde
vivem, tudo isso é parecido em média na vizinhança imediata do corte. Quem
fica de cada lado da linha foi separado por um detalhe quase fortuito. Se
nessa vizinhança aparecer uma diferença sistemática na frequência escolar,
ela vem do programa. É o tipo de comparação que se aproxima do que um
experimento daria, sem que ninguém tenha sido sorteado.

A premissa que sustenta o argumento é continuidade. A relação entre renda
e frequência escolar pode ser decrescente, ruidosa, irregular ao longo da
distribuição inteira. O método não precisa que ela seja linear, nem suave,
nem nada parecido. Só precisa que seja contínua, sem saltos, exceto onde o
programa muda discretamente. O efeito do tratamento é estimado como o
salto da curva no ponto da regra. Tudo o mais é continuidade. Só o
programa muda em degrau.

Esse arranjo aparece em mais lugares do que parece. ProUni com nota mínima
do Enem. Idade de corte para entrada na escola. Pontos de IDH acima dos
quais um município passa a receber recursos federais. Limite de tamanho a
partir do qual uma empresa passa a ter obrigações trabalhistas adicionais.
Cada vez que há uma regra explícita separando quem é tratado de quem não
é, e a regra é seguida com alguma fidelidade, há uma RDD possível.

Este guia trata da identificação (quando essa lógica vale), da estimação
na prática (regressão local polinomial, escolha de bandwidth, escolha do
polinômio), dos testes de validade (manipulação da regra pelos
beneficiários, descontinuidades em outras variáveis no mesmo ponto), e das
duas variantes principais. Sharp RDD se aplica quando a regra é seguida à
risca. Fuzzy RDD, quando o cumprimento é apenas parcial e a probabilidade
de tratamento salta no corte sem chegar a 100\%. O guia também é honesto
sobre o limite mais sério do método: o efeito estimado vale na vizinhança
do corte. Saber se o Bolsa Família afeta famílias muito mais pobres ou
muito mais ricas que a linha de elegibilidade exige outra estratégia.
\newpage
...
\chapter{Introdu\c{c}\~ao e Motiva\c{c}\~ao}

\section{O Problema da Causalidade com Regras de Corte}

Uma das perguntas mais frequentes em avalia\c{c}\~ao de pol\'iticas p\'ublicas \'e: \textit{o que teria acontecido com os benefici\'arios caso n\~ao tivessem participado do programa?} Quando a participa\c{c}\~ao \'e determinada por sorteio, a resposta \'e direta. Mas a maioria das pol\'iticas p\'ublicas n\~ao funciona assim --- h\'a crit\'erios de sele\c{c}\~ao.

Em muitos casos esse crit\'erio toma a forma de uma \textbf{regra de corte}: tem acesso ao programa apenas quem possui uma vari\'avel cont\'inua --- renda, pontua\c{c}\~ao em teste, popula\c{c}\~ao ou idade --- abaixo (ou acima) de determinado valor. Indiv\'iduos imediatamente \`a esquerda e \`a direita desse limite s\~ao parecidos em quase tudo, mas diferem sistematicamente no acesso ao tratamento. \'E essa descontinuidade local que o RDD explora para identificar efeitos causais.
...
\section{A Ideia Central do RDD}

A l\'ogica do RDD parte de uma observa\c{c}\~ao simples. No ponto de corte $Z^*$, unidades com $Z$ levemente abaixo e levemente acima s\~ao quase id\^enticas em qualquer caracter\'istica relevante --- exceto pelo acesso ao tratamento, que muda abruptamente. A diferen\c{c}a nos resultados entre esses dois grupos, portanto, identifica o efeito causal do programa \citep{LeeLemieux2010}.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{fig0_intuicao_rdd.pdf}
\caption{Intui\c{c}\~ao do RDD: descontinuidade no ponto de corte}
\label{fig:intuicao}
\figmeta{Dados simulados.}{FGV CLEAR.}{A frequ\^encia escolar cresce suavemente com a renda per capita, exceto no ponto de corte (R\$\,218) onde o acesso ao Bolsa Fam\'ilia gera uma descontinuidade. Nesta conven\c{c}\~ao, fam\'ilias eleg\'iveis t\^em $x > 0$ (lado direito do gr\'afico) e apresentam frequ\^encia escolar mais elevada do que as n\~ao eleg\'iveis ($x < 0$). O LATE do programa \'e positivo: a descontinuidade reflete o aumento na frequ\^encia escolar causado pelo benef\'icio.}
\end{figure}

\section{Breve Hist\'orico e Aplica\c{c}\~oes}

A formula\c{c}\~ao original do RDD vem de \cite{ThistlethwaiteCampbell1960}, em contexto educacional. O m\'etodo s\'o ganharia tra\c{c}\~ao d\'ecadas depois, com a formaliza\c{c}\~ao da identifica\c{c}\~ao por \cite{Hahn2001} e a consolida\c{c}\~ao do arcabou\c{c}o pr\'atico em \cite{ImbensLemieux2008} e \cite{LeeLemieux2010}. Aplica\c{c}\~oes que se tornaram refer\^encia da literatura:
...
\section{RDD no Ecossistema de M\'etodos de Avalia\c{c}\~ao}
...
\begin{conexaobox}
\textit{Guia de DiD --- DiD para pol\'iticas com varia\c{c}\~ao temporal vs RDD para pol\'iticas com regra de corte; Guia de SCM --- SCM para poucas unidades tratadas vs RDD para muitas unidades com regra de corte; Guia de Matching --- Arcabou\c{c}o causal geral.}
\end{conexaobox}
...
\section{Modelo de Resultados Potenciais}
...
\section{A \textit{Running Variable} e o Cutoff}
...
\'E conveniente trabalhar com a \textit{running variable} \textbf{centralizada}: $x_i = Z^* - Z_i$, de modo que o cutoff fique em $x = 0$. Unidades eleg\'iveis t\^em $x_i > 0$; n\~ao eleg\'iveis, $x_i < 0$ (no Bolsa Fam\'ilia: $x_i = \text{R}\$\,218 - \text{renda}_i$).

\section{Hip\'oteses de Identifica\c{c}\~ao}
...
\section{Estima\c{c}\~ao por Regress\~ao Local Polinomial}
...
\begin{conexaobox}
\textit{Guia de Avalia\c{c}\~ao de Impacto (Vol.~2) --- Se\c{c}\~ao 3 (Modelo de resultados potenciais, ATE vs ATT vs LATE); Guia de Matching --- Se\c{c}\~ao 2 (Compara\c{c}\~ao de estimadores locais vs globais).}
\end{conexaobox}
...
A probabilidade de tratamento passa de 0 para 1 \textit{no} ponto de corte: quem est\'a abaixo \'e tratado, quem est\'a acima n\~ao.

\section{Estima\c{c}\~ao com \texttt{rdrobust}}
...
# Sharp RDD: x = 218 - renda_per_capita (elegivel quando x > 0)
rdd_sharp <- rdrobust(
  y        = df$freq_escolar,
  x        = df$x,
...
\section{Interpretando o Output}
...
A estimativa a reportar \'e a \textbf{Robust}, n\~ao a convencional --- ela incorpora a corre\c{c}\~ao de vi\'es de segunda ordem e o IC reportado \'e v\'alido sob essa corre\c{c}\~ao. Formula\c{c}\~ao t\'ipica: ``O LATE estimado \'e de 8,0 p.p. (IC\,95\%: [5,4;\,10,6], $p < 0{,}01$).''
\end{notabox}

\section{O Gr\'afico RDD}
...
rdplot(y = df$freq_escolar, x = df$x, c = 0,
       ...
       x.label = "Renda per capita centralizada (R$ 218 - R$)",
...
\label{fig:sharp}
\figmeta{Dados simulados.}{FGV CLEAR.}{Cada ponto representa a m\'edia de frequ\^encia escolar num bin. As curvas s\~ao ajustes locais por lado do cutoff (geradas com \texttt{rdplot}). As linhas pontilhadas delimitam a janela MSE-\'otima. Fam\'ilias eleg\'iveis ($x > 0$) apresentam frequ\^encia escolar mais elevada (lado direito): a descontinuidade em $x = 0$ estima o LATE positivo do programa.}
\end{figure}

O gr\'afico confirma uma descontinuidade positiva e estatisticamente significativa na frequ\^encia escolar no cutoff de R\$\,218. O estimador robusto de \cite{Calonico2014} recupera um LATE pr\'oximo ao efeito verdadeiro de 8 p.p.\ implantado na simula\c{c}\~ao --- fam\'ilias com renda per capita pr\'oxima ao limiar e que acessam o Bolsa Fam\'ilia apresentam frequ\^encia escolar 8,0 pontos percentuais maior do que fam\'ilias similares sem o benef\'icio.
...
\section{Estima\c{c}\~ao por Vari\'aveis Instrumentais}

O Fuzzy RDD \'e estimado por \textbf{vari\'aveis instrumentais} (IV/2SLS), usando a elegibilidade $\mathbf{1}[Z_i \leq Z^*]$ como instrumento para o recebimento efetivo $D_i$. O par\^ametro de interesse --- o \textbf{estimando} --- \'e o LATE para os \textit{compliers} no cutoff, identificado pelo quociente entre a forma reduzida e o primeiro est\'agio:
\begin{equation}
  \tau_{Fuzzy} =
  \frac{
...
Sob a hip\'otese de monotonicidade (Hip\'otese~3 abaixo), essa express\~ao identifica o LATE para os \textit{compliers} no cutoff --- fam\'ilias que se cadastram \textit{porque} s\~ao eleg\'iveis \citep{Hahn2001}.
...
\section{Implementa\c{c}\~ao em R}
...
O Painel~A confirma o primeiro est\'agio: a probabilidade de receber o Bolsa Fam\'ilia salta de $\sim$5\% (n\~ao eleg\'iveis, $x < 0$) para $\sim$75\% (eleg\'iveis, $x > 0$) no cutoff --- evidenciando que a regra de elegibilidade \'e um instrumento forte para o recebimento efetivo. A estimativa IV (Painel~B) \'e ligeiramente maior do que a estimativa Sharp, pois o Fuzzy LATE mede o efeito especificamente sobre os \textit{compliers}: as fam\'ilias que se cadastram \textit{por causa} da elegibilidade, e que seriam as mais beneficiadas pela pol\'itica.
...
\begin{conexaobox}
\textit{Guia de Avalia\c{c}\~ao de Impacto (Vol.~2) --- Se\c{c}\~ao 4 (Vari\'aveis instrumentais: primeiro est\'agio, forma reduzida e interpreta\c{c}\~ao LATE para compliers).}
\end{conexaobox}
...
% ==========================================
\chapter{Testes de Validade e An\'alise de Robustez}

Toda estimativa de RDD precisa ser auditada empiricamente. As hip\'oteses de identifica\c{c}\~ao n\~ao s\~ao verific\'aveis diretamente, mas geram implica\c{c}\~oes test\'aveis. Esta se\c{c}\~ao apresenta os quatro testes que se tornaram padr\~ao na literatura \citep{ImbensLemieux2008,LeeLemieux2010}.

\section{Teste de Densidade: Aus\^encia de Manipula\c{c}\~ao}
...
# Nos dados simulados (sem manipulacao): p-valor esperado > 0.05
# -> nao rejeitar H0: sem evidencia de discontinuidade na densidade
# ATENCAO: p-valor < 0.05 REJEITARIA H0, indicando possivel manipulacao
\end{lstlisting}
...
\label{fig:density}
\figmeta{Dados simulados.}{FGV CLEAR.}{A continuidade da densidade em torno do cutoff (R\$\,218) sugere que as fam\'ilias n\~ao manipulam a renda declarada para se qualificarem. Nos dados simulados, o p-valor do teste de Cattaneo-Jansson-Ma deve ser superior a 0{,}05, indicando aus\^encia de evid\^encia de manipula\c{c}\~ao (H$_0$ n\~ao rejeitada). Um p-valor abaixo de 0{,}05 rejeitaria H$_0$ e sinalizaria poss\'ivel viola\c{c}\~ao da hip\'otese de descontinuidade.}
\end{figure}

\section{Balanceamento de Covari\'aveis}
...
\section{Sensibilidade ao \textit{Bandwidth}}

A escolha da janela MSE-\'otima envolve um \textit{trade-off} entre vi\'es e vari\^ancia, e a sensibilidade do resultado a essa escolha \'e parte da auditoria. Estimativas que se desfazem com varia\c{c}\~oes moderadas em $h$ devem ser tratadas como provis\'orias:
...
% ==========================================
\chapter{Aplica\c{c}\~ao Emp\'irica: Bolsa Fam\'ilia}

\section{Contexto do Programa}
...
\section{Design da Avalia\c{c}\~ao}
...
A dificuldade emp\'irica central est\'a na qualidade da \textit{running variable}: a renda registrada no Cad\'Unico difere da renda efetiva, e os erros de medi\c{c}\~ao da\'i resultantes atenuam as estimativas (\textit{attenuation bias}).

\section{Dados Simulados}

Os dados desta aplica\c{c}\~ao s\~ao simulados para fins did\'aticos e est\~ao dispon\'iveis no arquivo \texttt{bolsafamilia\_rdd.csv}, gerado pelo script \texttt{Cap\_RDD\_BolsaFamilia.R} que acompanha este guia. O script tamb\'em gera todas as figuras usadas aqui:
...
Os resultados do Sharp RDD e do Fuzzy RDD est\~ao discutidos nas Se\c{c}\~oes~\ref{sec:sharp} e~\ref{sec:fuzzy}, respectivamente, logo ap\'os as figuras correspondentes. Esta subse\c{c}\~ao apresenta os resultados para o segundo cutoff natural do Bolsa Fam\'ilia --- a linha de extrema pobreza --- que complementa a an\'alise principal.

\subsection{M\'ultiplos Cutoffs --- Efeitos Heterog\^eneos}
...
\begin{conexaobox}
\textit{Guia de DiD --- \cite{Ribeiro2017} e \cite{Chitolina2016} usam DiD para avaliar expans\~oes do Bolsa Fam\'ilia ao longo do tempo; \cite{Romero2009} usa RDD para estimar o efeito no ponto de corte.}
\end{conexaobox}
...
\section{M\'ultiplos Cutoffs}

Muitas pol\'iticas possuem mais de um ponto de corte, criando m\'ultiplas descontinuidades que podem ser exploradas para estimar \textbf{efeitos heterog\^eneos} \citep{CattaneoIdroboTitiunik2019}. A abordagem padr\~ao \'e estimar o RDD separadamente em cada cutoff, usando a \textit{running variable} centralizada no cutoff correspondente. Quando dois cutoffs est\~ao pr\'oximos, as janelas de estima\c{c}\~ao podem se sobrepor --- e nesse caso o efeito identificado em cada um deles fica contaminado pela presen\c{c}a do outro.

\section{RDD Geogr\'afico}

Fronteiras espaciais --- entre munic\'ipios, estados ou pa\'ises --- s\~ao tamb\'em descontinuidades, e o RDD pode trat\'a-las como tal. A \textit{running variable} \'e a dist\^ancia \`a fronteira, e o tratamento muda no momento em que essa dist\^ancia muda de sinal. As aplica\c{c}\~oes mais comuns envolvem pol\'iticas estaduais e municipais; estudos sobre conflitos em regi\~oes fronteiri\c{c}as tamb\'em recorrem ao desenho. Dois cuidados s\~ao mais delicados aqui do que no RDD usual: a fronteira deve ser exogenamente determinada (n\~ao tra\c{c}ada \textit{em resposta} ao que se quer medir), e \'e preciso descartar mobilidade das unidades atravessando a linha em busca do tratamento.

\section{Regress\~ao Kink (\textit{Regression Kink Design})}

Algumas pol\'iticas n\~ao operam por salto, mas por mudan\c{c}a de inclina\c{c}\~ao. O seguro-desemprego \'e o exemplo cl\'assico: o benef\'icio cresce linearmente com a renda anterior at\'e um teto, e depois a inclina\c{c}\~ao muda --- mas o n\'ivel n\~ao salta. O RKD identifica o efeito da \textit{taxa} do tratamento explorando essa quina, em vez de uma descontinuidade no n\'ivel. Aplica\c{c}\~oes t\'ipicas incluem benef\'icios previdenci\'arios e programas com f\'ormulas escalonadas.
...
\section{RDD em Tempo Cont\'inuo}

Quando uma pol\'itica entra em vigor numa data espec\'ifica, o pr\'oprio tempo pode servir como \textit{running variable} e a data de implementa\c{c}\~ao como cutoff. A estrat\'egia funciona melhor com dados de alta frequ\^encia, em que os dois lados da data podem ser bem amostrados sem que choques sazonais contaminem a compara\c{c}\~ao.
...
\begin{conexaobox}
\textit{Guia de SCM --- RDD Geogr\'afico e SCM s\~ao complementares quando a pol\'itica tem cobertura geogr\'afica espec\'ifica; Guia de DiD --- Time RDD \'e alternativa ao DiD quando h\'a uma \'unica data de implementa\c{c}\~ao e dados de alta frequ\^encia.}
\end{conexaobox}
...
% ==========================================
\chapter{Armadilhas Comuns e Checklist}

\section{Armadilha 1: Polin\^omios de Alta Ordem}

Polin\^omios de grau 4 ou 5 produzem ajustes esp\'urios pr\'oximos ao cutoff --- curvam-se para cima ou para baixo exatamente onde est\'a o ponto de interesse, inflando ou deflando a estimativa do salto. \textbf{Diagn\'ostico:} comparar estimativas de ordem 1, 2 e 3; diverg\^encias grandes indicam resultado inst\'avel. \textbf{Solu\c{c}\~ao:} ordem 1 como padr\~ao, ordem 2 como robustez \citep{GelmanImbens2019}.

\section{Armadilha 2: Janela \'{U}nica sem Verifica\c{c}\~ao de Sensibilidade}

Reportar apenas a janela MSE-\'otima esconde do leitor o quanto o resultado depende daquela escolha. \textbf{Solu\c{c}\~ao:} sempre acompanhar a estimativa principal com a Tabela de Sensibilidade ao Bandwidth (Se\c{c}\~ao~5.4).

\section{Armadilha 3: Heaping na \textit{Running Variable}}
...
\section{Armadilha 4: Descontinuidades em Outras Pol\'iticas}
...
\section{Armadilha 5: Generalizar o LATE para Toda a Popula\c{c}\~ao}
...
\section{Tabela-Resumo: Diagn\'osticos e Solu\c{c}\~oes}
...
A Regress\~ao Descont\'inua se tornou refer\^encia na avalia\c{c}\~ao de pol\'iticas com elegibilidade definida por regra de corte. O guia percorreu o arcabou\c{c}o de identifica\c{c}\~ao, os casos Sharp e Fuzzy, os testes de validade, a aplica\c{c}\~ao ao Bolsa Fam\'ilia e as principais extens\~oes (m\'ultiplos cutoffs, RKD, RDD geogr\'afico e em tempo cont\'inuo).

Cinco pontos merecem ser retidos pelo leitor que pretende aplicar o m\'etodo. O primeiro \'e que a qualidade da \textit{running variable} \'e o ativo central: quanto mais precisa e ex\'ogena, mais cr\'ivel a estimativa. Da\'i a centralidade do segundo ponto: a hip\'otese de aus\^encia de manipula\c{c}\~ao precisa ser testada formalmente, com o teste de densidade, e n\~ao apenas justificada institucionalmente. O terceiro ponto diz respeito \`a interpreta\c{c}\~ao --- o RDD identifica um LATE, v\'alido na vizinhan\c{c}a do cutoff; generaliza\c{c}\~oes para toda a popula\c{c}\~ao eleg\'ivel s\~ao extrapola\c{c}\~oes que precisam ser explicitadas. O quarto, de natureza t\'ecnica, \'e o reporte: a estimativa \textbf{Robust} do \texttt{rdrobust} \'e o padr\~ao, e os quatro testes de validade da Se\c{c}\~ao~5 devem acompanh\'a-la. Por fim, o gr\'afico do RDD n\~ao \'e opcional: \'e ele que permite ao leitor enxergar a descontinuidade e avaliar a credibilidade do desenho antes mesmo de olhar para o coeficiente.

\begin{conexaobox}
\textit{Guia de SCM --- RDD para pol\'iticas com regra de corte vs SCM para pol\'iticas de n\'ivel agregado; Guia de DiD --- Converg\^encia crescente RDD-DiD quando h\'a m\'ultiplos cutoffs e varia\c{c}\~ao temporal; Guia de Matching --- O RDD pode ser visto como um pareamento local no cutoff.}
\end{conexaobox}
...
\bibitem[Card et al.(2015)]{Card2015}
Card, D., Lee, D.~S., Pei, Z. e Weber, A. (2015).
\newblock Inference on Causal Effects in a Generalized Regression Kink Design.
\newblock \textit{Econometrica}, 83(6), 2453--2483.
