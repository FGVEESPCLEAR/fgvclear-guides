...
%%  Guia de Controle Sintético (SCM)
%% ============================================================
...
\hypersetup{
  ...
  pdfauthor={FGV CLEAR},
  pdftitle={Guia de Controle Sintético (SCM)}
}
\usepackage{natbib}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[R]{\small\color{clearblue}\textit{Guia CLEAR $|$ Controle Sint\'etico (SCM)}}
\fancyfoot[C]{\thepage}
...
\vspace{2cm}
{\large FGV CLEAR $|$ FGV EESP Policy Lab}
...
\textbf{Equipe técnica:} André Portela Souza, Caio de Souza Castro, Fabrícia Volotão Peixoto, Luigi Garzon, Lycia Lima, Michel Szklo.
...
A Austrália implementou em 2012 a regra de embalagem genérica de cigarro:
maços padronizados, marca em fonte uniforme, imagens explícitas de doenças
cobrindo boa parte da superfície. A política era controversa e a indústria
contestou na justiça internacional. Saber se ela funcionou virou pergunta
de interesse global, com implicação direta para dezenas de outros países
considerando medida parecida.

Mas como avaliar? Há uma Austrália, uma intervenção, um momento. Comparar
com o Reino Unido, que tomou medida similar logo depois? Com os Estados
Unidos, que não tomaram? Com a Nova Zelândia, geograficamente próxima mas
com estrutura de mercado diferente? Cada escolha de comparação é
defensável de algum ângulo e atacável de outro. E a estimativa do efeito
acaba dependendo dessa escolha mais do que da própria política.

O método de controle sintético contorna essa arbitrariedade construindo o
contrafactual em vez de escolhê-lo. A ideia é simples na enunciação. Em
vez de escolher um país que sirva de comparação, monte um. Uma combinação
ponderada de vários países que, juntos, reproduzem a trajetória da
Austrália no período anterior à política. Os pesos são definidos pelos
dados: maximizam a aderência da combinação à série pré-política da unidade
tratada, e os países que entram na combinação são exatamente aqueles que,
juntos, mais se parecem com ela antes da intervenção. Se essa
``Austrália sintética'' acompanhou bem a real no período em que a política
ainda não estava em vigor, ela é uma referência razoável para o que teria
acontecido depois sem a política.

A inferência precisa ser pensada de outra forma. Não existe erro-padrão
clássico quando se observa uma realização única, e os intervalos de
confiança usuais perdem sentido. O teste no método é por placebo.
Aplica-se a mesma construção sintética a países que não foram tratados, e
compara-se a ``diferença pós-política'' de cada um deles em relação ao seu
próprio sintético com a diferença observada na Austrália real. Se em
muitos placebos a diferença chega a ser tão grande quanto a observada, a
estimativa pode ser ruído de série temporal. Se a Austrália se destaca
claramente, é evidência de efeito.

Este guia trata do método clássico de Abadie, Diamond e Hainmueller, das
extensões mais recentes (controle sintético aumentado, controle sintético
com diferenças-em-diferenças) que relaxam algumas restrições do desenho
original, e dos diagnósticos que separam um bom ajuste pré-política de um
ruim. A aplicação central usa a Lei Seca brasileira de 2008 como caso,
mostrando passo a passo a construção do sintético, a inferência via
placebo e a interpretação dos resultados. É o método indicado quando a
intervenção atinge poucas unidades, há série temporal longa antes da
política, e nenhum candidato a comparação direta é claramente bom.
\newpage
...
\section{O Problema de Avaliação com Poucos Tratados}
...
\begin{conexaobox}
\textit{Ver Guia de Avaliação de Impacto --- Seção sobre métodos quasi-experimentais; Guia de DiD --- Seção 1 (Hipótese de tendências paralelas e suas limitações).}
\end{conexaobox}


\section{A Ideia Central do Controle Sintético}

A intuição do SCM é simples. Em vez de escolher uma \textit{única} unidade de controle (por exemplo, usar a Argentina como controle para o Brasil), o método constrói um \textbf{controle sintético}: uma combinação ponderada de várias unidades não tratadas que, juntas, reproduzem o mais fielmente possível a trajetória da unidade tratada \textit{antes} da intervenção.
...
\begin{conexaobox}
\textit{Ver Guia de DiD --- Seção 1 (Comparação: SCM não requer a hipótese de tendências paralelas); Guia de Matching --- Seção 3 (Pareamento como construção de contrafactuais; SCM como extensão do matching para painel).}
\end{conexaobox}


\section{Exemplos Canônicos}
...
\section{SCM no Ecossistema de Métodos de Avaliação}
...
\begin{conexaobox}
\textit{Ver Guia de RDD --- Seção 1.3 (RDD no ecossistema); Guia de Matching --- Seção 1 (Arcabouço causal geral).}
\end{conexaobox}

\section{Por Que Não Usar DiD? O Problema das Tendências Paralelas}
...
A comparação revela uma complementaridade fundamental: o DiD é robusto a diferenças \textit{permanentes} de nível entre unidades (via efeitos fixos), enquanto o SCM é robusto a diferenças na \textit{dinâmica temporal} (via ajuste de trajetória). O SDID (Seção~3.5) combina ambas as vantagens.

\begin{conexaobox}
\textit{Ver Guia de DiD --- Seção 1 (Hipótese de tendências paralelas e testes de pré-tendência); Guia de Matching --- Seção 2 (Hipótese de seleção em observáveis vs não-observáveis).}
\end{conexaobox}
...
\section{Modelo de Resultados Potenciais e o Modelo Fatorial}
...
\begin{conexaobox}
\textit{Ver Guia de DiD --- Seção 1.1 (Modelo de resultados potenciais e a importância de definir bem o estimando); Seção 2 (DiD escalonado e modelo fatorial; relação com TWFE).}
\end{conexaobox}


\section{O Resultado Central: Por Que o SCM Funciona}
...
\section{O Estimador SCM Clássico}
...
\subsection{A Escolha da Matriz de Pesos V}
...
\subsection{Propriedades dos Pesos}
...
\subsection{O Estimador do Efeito Causal}
...
\subsection{Código em R: Exemplo com a Proposição 99}
...
\begin{conexaobox}
\textit{Ver Guia de DiD --- Seção 3 (Padrões de código em R e boas práticas de replicação).}
\end{conexaobox}


\section{Inferência no SCM Clássico}

A inferência é o aspecto mais delicado do SCM. Ao contrário de métodos com muitas unidades, onde a teoria assintótica é natural, o SCM opera tipicamente com uma única unidade tratada e um pool modesto de doadores --- e não há, em geral, distribuição assintótica do estimador à qual recorrer.

\subsection{Testes Placebo In-Space}
...
\subsection{Testes Placebo In-Time}
...
\subsection{P-valores e Razão RMSPE}
...
\section{Quando o SCM Falha: Diagnósticos e Limitações}
...
% ==========================================
\newpage
\chapter{Extensões e Estimadores Recentes}
...
\section{SCM Aumentado (ASCM) --- Ben-Michael, Feller e Rothstein (2021)}

\subsection{Motivação: O Problema do Ajuste Imperfeito}
...
\subsection{Código em R com \texttt{augsynth}}
...
\section{SCM Penalizado --- Abadie e L'Hour (2021)}
...
\section{Controle Sintético Generalizado (GSC) --- Xu (2017)}

\subsection{Motivação: Múltiplos Tratados}
...
\subsection{Código em R com \texttt{gsynth}}
...
\begin{conexaobox}
\textit{Ver Guia de DiD --- Seção 3.3 (De Chaisemartin e d'Haultfœuille, 2020; comparação de estimadores robustos para DiD com tratamento heterogêneo).}
\end{conexaobox}


\section{Matrix Completion --- Athey et al.\ (2021)}
...
\section{Synthetic Difference-in-Differences (SDID) --- Arkhangelsky et al.\ (2021)}
...
\begin{conexaobox}
\textit{Ver Guia de DiD --- Seção 2 (DiD escalonado e motivação para pesos que corrigem vieses); a convergência entre DiD e SCM via SDID é uma das principais tendências recentes.}
\end{conexaobox}


\section{Staggered Synthetic Control}
...
\subsection{Partially Pooled SCM --- Ben-Michael, Feller e Rothstein (2022)}
...
\subsection{Intervalos de Predição --- Cattaneo, Feng, Palomba e Titiunik (2025)}
...
\section{Comparação entre os Estimadores}
...
% ==========================================
\newpage
\chapter{Avanços na Fronteira da Pesquisa}

\section{Inferência Conformal para SCM}
...
\section{Outcomes Distribucionais}
...
\section{SCM e Machine Learning}
...
% ==========================================
\newpage
\chapter{Replicação Completa: A Lei Seca Brasileira}

\section{Descrição da Política e do Contexto}
...
O Brasil, na época da implementação, apresentava uma taxa de mortalidade no trânsito de aproximadamente 20 óbitos por 100.000 habitantes --- bem acima da de países como Chile ($\sim$12) e Argentina ($\sim$14), segundo dados da OMS. Estima-se que a Lei Seca salvou mais de 40.000 vidas entre 2008 e 2016.
...
\section{Preparação e Código em R}
...
\subsection{Composição do Brasil Sintético}
...
\subsection{Magnitude do Efeito}
...
\begin{conexaobox}
\textit{Ver Guia de Custo-Benefício --- Como usar estimativas de impacto agregado em análise de custo-efetividade (e.g., custo por vida salva pela Lei Seca).}
\end{conexaobox}
...
% ==========================================
\newpage
\chapter{Checklist de Boas Práticas e Armadilhas Comuns}

\section{Checklist para Praticantes}
...
\subsection{Armadilha 1: Usar SCM com ajuste pré-tratamento ruim sem extensões}
...
\subsection{Armadilha 2: Incluir unidades contaminadas no pool de doadores}
...
\subsection{Armadilha 3: Não reportar os pesos}
...
\subsection{Armadilha 4: Overfitting pré-tratamento}
...
\subsection{Armadilha 5: Ignorar spillovers}

Se a política da unidade tratada afeta as unidades de controle, o contrafactual é inválido. \textbf{Exemplo:} ao avaliar o impacto da Lei Seca no Brasil, se o turismo rodoviário de brasileiros em países vizinhos muda, os controles não representam o ``mundo sem Lei Seca''. \textbf{Solução:} argumente por que spillovers são implausíveis; use extensões com spillovers (Seção~4.2) quando apropriado.

\subsection{Armadilha 6: P-valores inflados com pool pequeno}
...
\subsection{Armadilha 7: Generalizar indevidamente}
...
\section{Tabela-Resumo: Diagnósticos e Soluções}
...
O controle sintético se consolidou como ferramenta de referência para avaliação de impacto quando há poucas unidades tratadas. Este guia cobriu o arcabouço teórico (Seção~2), as extensões mais recentes (Seções~3--4) e uma replicação empírica com dados brasileiros (Seção~5).

Cinco pontos atravessam o guia. O primeiro é o nicho do método: o SCM é adequado para intervenções de nível agregado, com uma ou poucas unidades tratadas --- aí estão suas vantagens comparativas, não em qualquer aplicação. O segundo é diagnóstico: a qualidade do ajuste pré-tratamento é o teste mais importante de uma análise SCM, e quando o ajuste é ruim, o ASCM corrige parte do viés. O terceiro é interpretativo: a transparência dos pesos --- saber exatamente quais unidades compõem o controle sintético, e com que peso --- é uma vantagem competitiva em relação a métodos de caixa-preta, e deve ser sempre reportada. O quarto é a convergência com a literatura de DiD: o SDID oferece uma ponte natural entre os dois métodos, e o Staggered SCM estende o controle sintético para adoção escalonada. O quinto, finalmente, é a inferência, que permanece o aspecto mais desafiador --- com a inferência conformal e os intervalos de predição de \citet{CattaneoFengPalombaTitiunik2025} como avanços recentes.

Direções futuras incluem a integração com \textit{machine learning} (modelos de resultados não-paramétricos no ASCM), a extensão para \textit{outcomes} distribucionais via \textit{optimal transport}, e a convergência crescente com a literatura de DiD.

\begin{conexaobox}
\textit{Guia de RDD --- Comparação: RDD para políticas com regra de corte vs SCM para políticas de nível agregado; Guia de DiD --- Convergência crescente DiD-SCM via Synthetic DiD e Staggered designs.}
\end{conexaobox}
