...
\section{Avaliando um Programa de Treinamento}
...
Por fim, iremos criar algumas variáveis que utilizaremos em análises mais adiante.
...
\section{Resultados Potenciais}\label{po}
...
A Equação \eqref{viesselecao} nos mostra que a resposta é não — pelo menos por enquanto. Na ausência de uma hipótese explícita sobre o processo de alocação no programa NSW, a simples comparação de médias entre o grupo de tratamento e o grupo de controle estará potencialmente enviesada. O viés de seleção, definido como $\mathbb{E}[Y_i(0) \mid D_i = 1] - \mathbb{E}[Y_i(0) \mid D_i = 0]$, reflete a presença de características individuais que influenciam simultaneamente a decisão de participar do programa e os ganhos monetários potenciais.


Imagine, por exemplo, que a divulgação do programa NSW tenha atingido majoritariamente pessoas de maior escolaridade. Assim, podemos supor que a probabilidade de alguém ser tratado aumenta quanto maior for sua escolaridade. Além disso, pessoas com maior escolaridade tendem a ter ganhos monetários maiores. Podemos visualizar as direções da causalidade no grafo abaixo:
...
Neste caso, ao compararmos o grupo que participou do NSW com o grupo que não participou, estamos essencialmente comparando os ganhos monetários de pessoas com maior escolaridade com os de pessoas com menor escolaridade. Se acreditarmos que os ganhos monetários desses dois grupos é diferente, o que é totalmente plausível, então $\mathbb{E}[Y_i(0) \mid D_i = 1] - \mathbb{E}[Y_i(0) \mid D_i = 0] \neq 0$, ou seja, existe viés de seleção.

\section{Experimentos}

O diferencial do NSW é que ele foi um experimento aleatorizado: entre os candidatos elegíveis, alguns foram escolhidos aleatoriamente para participar, enquanto outros não foram selecionados e serviram como grupo de controle. Isso permite avaliar de forma rigorosa o efeito causal do programa sobre a renda e o emprego dos participantes.
...
Podemos estimar esses parâmetros causais de duas maneiras: por meio de uma diferença de médias simples ou por meio da regressão dos ganhos monetários na participação no programa. A diferença de médias, nesse caso, é equivalente ao parâmetro de Mínimos Quadrados Ordinários (MQO) relacionado à variável de tratamento.\footnote{Apêndice} Aqui, iremos estimar esses efeitos através da seguinte regressão:
$$
...
A Tabela \ref{reg1} mostra os resultados da regressão. Ela indica que o programa NSW \textbf{causou}, em média, um aumento de \$1.794,342 para os participantes do programa.
...
\section{Dados Não-Experimentais}
